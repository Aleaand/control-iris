...
<div class="p-6 space-y-6">
    ...
    <div class="flex items-start justify-between">
        ...
        <div class="flex items-center gap-2">
            @if($urgentFlights > 0)
                <div class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-rose-500/10 border border-rose-500/30 animate-pulse">
                    <div class="w-1.5 h-1.5 rounded-full bg-rose-400"></div>
                    <span class="font-mono-tech text-[9px] text-rose-400 uppercase">{{ $urgentFlights }} vuelo(s) en 72h</span>
                </div>
            @endif
            @if($pendingTasks > 0)
                <a href="{{ route('gestor.missions') }}" wire:navigate
                   class="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-orange-500/10 border border-orange-500/30 hover:bg-orange-500/20 transition-colors">
                    ...
                    <span class="font-mono-tech text-[9px] text-orange-400 uppercase">{{ $pendingTasks }} misión(es)</span>
                </a>
            @endif
        </div>
    </div>
    ...
    <div class="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-6 gap-4">
        ...
        <a href="{{ route('gestor.clients') }}" wire:navigate
           class="tech-card p-4 rounded-xl hover:border-cyan-500/30 transition-all duration-200 group">
            <div class="flex items-center justify-between mb-3">
                <div class="w-8 h-8 rounded-lg bg-cyan-500/10 flex items-center justify-center text-cyan-400 group-hover:scale-110 transition-transform">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><path d="M23 21v-2a4 4 0 0 0-3-3.87"></path><path d="M16 3.13a4 4 0 0 1 0 7.75"></path></svg>
                </div>
            </div>
        ...
        <a href="{{ route('gestor.reservations') }}" wire:navigate
           class="tech-card p-4 rounded-xl hover:border-violet-500/30 transition-all duration-200 group">
            <div class="flex items-center justify-between mb-3">
                <div class="w-8 h-8 rounded-lg bg-violet-500/10 flex items-center justify-center text-violet-400 group-hover:scale-110 transition-transform">
        ...
        <a href="{{ route('gestor.payments') }}" wire:navigate
           class="tech-card p-4 rounded-xl transition-all duration-200 group {{ $pendingPayments > 0 ? 'border-amber-500/30 hover:border-amber-500/50' : 'hover:border-white/10' }}">
            <div class="flex items-center justify-between mb-3">
                <div class="w-8 h-8 rounded-lg {{ $pendingPayments > 0 ? 'bg-amber-500/10 text-amber-400' : 'bg-zinc-700/20 text-zinc-500' }} flex items-center justify-center group-hover:scale-110 transition-transform">
            ...
            <p class="text-2xl font-black {{ $pendingPayments > 0 ? 'text-amber-400' : '' }}" style="{{ $pendingPayments === 0 ? 'color: var(--text-primary)' : '' }}">{{ $pendingPayments }}</p>
            <p class="font-mono-tech text-[9px] text-zinc-500 uppercase mt-0.5">Pagos Pendientes</p>
        </a>

        {{-- Vuelos en 72h --}}
        <a href="{{ route('gestor.compliance') }}" wire:navigate
           class="tech-card p-4 rounded-xl transition-all duration-200 group {{ $urgentFlights > 0 ? 'border-rose-500/30' : 'hover:border-white/10' }}">
            <div class="flex items-center justify-between mb-3">
                <div class="w-8 h-8 rounded-lg {{ $urgentFlights > 0 ? 'bg-rose-500/10 text-rose-400' : 'bg-zinc-700/20 text-zinc-500' }} flex items-center justify-center group-hover:scale-110 transition-transform">
            ...
            <p class="text-2xl font-black {{ $urgentFlights > 0 ? 'text-rose-400' : '' }}" style="{{ $urgentFlights === 0 ? 'color: var(--text-primary)' : '' }}">{{ $urgentFlights }}</p>
        ...
        <a href="{{ route('gestor.missions') }}" wire:navigate
           class="tech-card p-4 rounded-xl transition-all duration-200 group {{ $pendingTasks > 0 ? 'border-orange-500/30' : 'hover:border-white/10' }}">
            <div class="flex items-center justify-between mb-3">
                <div class="w-8 h-8 rounded-lg {{ $pendingTasks > 0 ? 'bg-orange-500/10 text-orange-400' : 'bg-zinc-700/20 text-zinc-500' }} flex items-center justify-center group-hover:scale-110 transition-transform">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><polyline points="9 11 12 14 22 4"></polyline><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"></path></svg>
                </div>
            </div>
            <p class="text-2xl font-black {{ $pendingTasks > 0 ? 'text-orange-400' : '' }}" style="{{ $pendingTasks === 0 ? 'color: var(--text-primary)' : '' }}">{{ $pendingTasks }}</p>
            <p class="font-mono-tech text-[9px] text-zinc-500 uppercase mt-0.5">Misiones</p>
        </a>
    ...
    <div class="grid grid-cols-1 xl:grid-cols-3 gap-6">
        ...
        <div class="xl:col-span-2 tech-card rounded-xl p-5">
            <div class="flex items-center justify-between mb-4">
                ...
                <a href="{{ route('gestor.reservations') }}" wire:navigate class="font-mono-tech text-[9px] text-emerald-400 hover:text-emerald-300 uppercase tracking-wider transition-colors">
                    Ver todas →
                </a>
            </div>
                        ...
                        <div class="flex items-center gap-3 px-3 py-2.5 rounded-lg hover:bg-white/3 transition-colors">
                            ...
                            <div class="flex-1 min-w-0">
                                ...
                                <p class="font-mono-tech text-[9px] text-zinc-500">
                                    {{ $res->spaceFlight?->destination?->name ?? '—' }} · {{ $res->spaceFlight?->departure_date?->format('d/m/Y') ?? '—' }}
                                </p>
                            </div>
                            <div class="text-right flex-shrink-0">
                                <span class="font-mono-tech text-[9px] px-2 py-0.5 rounded-full
        ...
        {{-- Panel lateral: Misiones urgentes + Próximos vuelos --}}
        <div class="space-y-6">
            ...
            @if($urgentTasks->isNotEmpty())
                <div class="tech-card rounded-xl p-5" style="border-color: rgba(249,115,22,0.2)">
                    <div class="flex items-center gap-2 mb-4">
                        ...
                        <h2 class="font-mono-tech text-[11px] uppercase tracking-widest text-orange-400">Misiones Urgentes</h2>
                    </div>
                    <div class="space-y-2">
                    ...
                    <div class="space-y-2">
                        @foreach($upcomingFlights as $res)
                            @php
                                ...
                                $isUrgent = $daysLeft <= 3;
                            @endphp
                            <div class="flex items-center gap-2 px-2 py-2 rounded-lg {{ $isUrgent ? 'bg-rose-500/5 border border-rose-500/10' : '' }}">
                                <div class="flex-1 min-w-0">
                                    <p class="text-xs font-semibold truncate" style="color: var(--text-primary)">
                                        {{ $res->passenger?->full_name ?? $res->user?->name ?? 'N/A' }}
                                    </p>
                                ...
                                @if($isUrgent)
                                    <span class="font-mono-tech text-[8px] text-rose-400 bg-rose-500/10 px-1.5 py-0.5 rounded">{{ $daysLeft }}d</span>
                                @endif
                            </div>
                        @endforeach
                    </div>

...
">
    ...
    <aside class="sidebar" :class="{ 'open': openMobile, 'collapsed': isCollapsed }">
        ...
        <!-- Badge de rol -->
        <div x-show="!isCollapsed" class="px-4 mt-4 mb-4" x-transition:enter="transition ease-out duration-300"
        ...
        <!-- Navigation Menu -->
        <nav class="nav-menu custom-scrollbar overflow-y-auto md:mt-0">
            ...
            <a href="{{ route('gestor.clients') }}" wire:navigate class="nav-item hover-cyan"
                ...
                <span class="nav-text" x-show="!isCollapsed" x-transition:enter="transition ease-out duration-300"
                    ...
                    x-transition:enter-end="opacity-100 translate-x-0"
                    x-transition:leave="transition ease-in duration-700" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0">Mis Clientes</span>
            </a>

            <!-- Central de Reservas -->
            <a href="{{ route('gestor.reservations') }}" wire:navigate class="nav-item" style="--hover-color: #10b981"
            ...
            <!-- Documentación -->
            <a href="{{ route('gestor.documentation') }}" wire:navigate class="nav-item hover-violet"
                :class="'{{ request()->routeIs('gestor.documentation') }}' === '1' ? 'active text-violet-500' : ''">
                <svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path
                ...
                <span class="nav-text" x-show="!isCollapsed" x-transition:enter="transition ease-out duration-300"
                    ...
                    x-transition:enter-end="opacity-100 translate-x-0"
                    x-transition:leave="transition ease-in duration-700" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0">Documentación</span>
            </a>
            ...
            <a href="{{ route('gestor.payments') }}" wire:navigate class="nav-item hover-amber"
                ...
                <span class="nav-text" x-show="!isCollapsed" x-transition:enter="transition ease-out duration-300"
                    ...
                    x-transition:enter-end="opacity-100 translate-x-0"
                    x-transition:leave="transition ease-in duration-700" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0">Pagos & Reembolsos</span>
            </a>
            ...
            <a href="{{ route('gestor.radar') }}" wire:navigate class="nav-item hover-rose"
                ...
                <span class="nav-text" x-show="!isCollapsed" x-transition:enter="transition ease-out duration-300"
                    ...
                    x-transition:enter-end="opacity-100 translate-x-0"
                    x-transition:leave="transition ease-in duration-700" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0">Radar de Recursos</span>
            <a href="{{ route('gestor.tasks') }}" wire:navigate class="nav-item hover-orange"
                :class="'{{ request()->routeIs('gestor.tasks') }}' === '1' ? 'active text-orange-500' : ''">
                <svg class="nav-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                ...
                <span class="nav-text" x-show="!isCollapsed" x-transition:enter="transition ease-out duration-300"
                    ...
                    x-transition:enter-end="opacity-100 translate-x-0"
                    x-transition:leave="transition ease-in duration-700" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0">Gestión de Tareas
                    @php $pendingTasks = \App\Models\Task::where('assigned_gestor_id', auth()->id())->where('status', 'Pendiente')->count(); @endphp
                    @if($pendingTasks > 0)
                        <span
                            class="ml-auto bg-orange-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full">{{ $pendingTasks }}</span>
                    @endif
                </span>
            </a>
            ...
            <a href="{{ route('gestor.communication') }}" wire:navigate class="nav-item hover-blue"
                ...
                <span class="nav-text" x-show="!isCollapsed" x-transition:enter="transition ease-out duration-300"
                    ...
                    x-transition:enter-end="opacity-100 translate-x-0"
                    x-transition:leave="transition ease-in duration-700" x-transition:leave-start="opacity-100"
                    x-transition:leave-end="opacity-0">Comunicación</span>
            </a>
        ...
        <div class="user-bubble-container" x-cloak>
            ...
            <div x-show="openUser" x-cloak @click.away="openUser = false"
                ...
                class="user-dropdown-panel z-50" style="left: 85px; bottom: 24px;">
                <a href="{{ route('gestor.profile') }}" wire:navigate
                    class="w-full flex items-center px-4 py-3 text-xs text-zinc-400 hover:text-[var(--text-primary)] hover:bg-white/5 rounded-lg mb-1 transition-all group/profile">
                    <span>Mi Perfil</span>
                </a>
        ...
        <div x-show="showLogoutModal" x-cloak class="fixed inset-0 z-[300] flex items-center justify-center p-4">
            ...
            <div class="tech-card p-8 w-full max-w-sm relative overflow-hidden" x-show="showLogoutModal"
                ...
                <div class="flex flex-col items-center text-center">
                    <div
                        class="w-16 h-16 rounded-2xl bg-rose-500/10 flex items-center justify-center text-rose-500 mb-6 border border-rose-500/20">

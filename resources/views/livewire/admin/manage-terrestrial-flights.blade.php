<div class="p-6 md:p-8 space-y-6 relative obsidian-bg min-h-screen text-[var(--text-primary)]" x-data="{ showScrollTop: false }"
    @scroll.window="showScrollTop = window.pageYOffset > 300"> <!-- Header & Flash Message -->
    <div class="flex flex-col md:flex-row justify-between items-start md:items-end border-b border-[var(--neon-rose)]/30 pb-4">
        <div>
    ...
    <div class="grid grid-cols-1 md:grid-cols-5 gap-y-4 md:gap-8 items-start md:grid-rows-[auto_auto_1fr]">
        ...
        <div class="md:col-span-3 md:col-start-1 md:row-start-1">
            <div class="tech-card p-4 flex flex-col sm:flex-row gap-4 justify-between items-center rounded-[10px] shadow-lg">
                <div class="relative w-full sm:w-2/3">
                    ...
                    <input type="text" wire:model.live="search" placeholder="Búsqueda..."
                        class="tech-input block w-full pl-10 py-2 focus:outline-none sm:text-sm transition-colors rounded-[10px]">
                </div>
        ...
        <div class="md:col-span-2 md:col-start-4 md:row-start-1 md:row-span-3 mt-4 md:mt-0"
            ...
            <div
                class="tech-card p-6 rounded-[10px] shadow-lg transition-all duration-500 animate-slide-in {{ $isEditing ? 'border-2 border-[var(--neon-amber)] shadow-[0_0_20px_rgba(245,158,11,0.1)]' : 'border-2 border-[var(--border-glass)]' }}">
                <!-- Mobile Toggle -->
                <button @click="expanded = !expanded" type="button"
                    ...
                    <span x-text="expanded ? 'Ocultar Formulario' : '{{ $isEditing ? 'Continuar Edición' : 'Nuevo Vuelo Terrestre' }}'"></span>
                ...
                <div x-show="expanded" x-transition>
                    ...
                    <div
                        class="flex justify-between items-center mb-6 border-b border-[var(--border-glass)] pb-4">
                        <h3
                    ...
                    <form wire:submit.prevent="confirmSave" class="space-y-4">
                        ...
                        <div class="pt-4 mt-2 border-t border-[var(--border-glass)]">
                            <button type="submit"
                                class="w-full {{ $isEditing ? 'bg-[var(--neon-amber)] hover:bg-[var(--neon-amber)] text-black border-[var(--neon-amber)] shadow-[0_0_15px_rgba(245,158,11,0.3)]' : 'bg-[var(--neon-rose)] hover:bg-[var(--neon-rose)] text-black border-[var(--neon-rose)] shadow-[0_0_15px_rgba(244,63,94,0.3)]' }} font-bold uppercase tracking-widest py-3 px-4 transition-all text-xs rounded-[10px] border flex items-center justify-center gap-2">
            ...
            <div class="tech-card rounded-[10px] shadow-lg overflow-hidden">
                <ul class="divide-y divide-[var(--border-glass)]">
                    @forelse($flights as $flight)
                        <li
                            class="p-4 hover:bg-[var(--tech-hover-bg)] transition-colors flex flex-col sm:flex-row justify-between sm:items-center gap-4 group">
                            <div class="flex-1">
                                <div class="flex items-center gap-3 mb-2">
                                    <span
                                        class="text-[9px] sm:text-xs font-mono text-[var(--text-secondary)] bg-[var(--tech-input-bg)] px-2 py-0.5 rounded-[5px] border border-[var(--border-glass)]">#{{ str_pad($flight->id, 4, '0', STR_PAD_LEFT) }}</span>
                                    <h4
                                ...
                                <div class="flex flex-wrap gap-2">
                                    ...
                                    <div
                                        ...
                                        <svg class="w-3.5 h-3.5 text-[var(--text-secondary)]" fill="none" stroke="currentColor"
                                            viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1"
                            ...
                            <div
                                class="flex sm:flex-col gap-2 shrink-0 border-t border-[var(--border-glass)] sm:border-0 pt-3 sm:pt-0">
        ...
        @if($showSaveModal)
            <div
                ...
                <div class="tech-card bg-[var(--bg-panel)] max-w-sm w-full overflow-hidden shadow-2xl"
                    @click.away="$wire.set('showSaveModal', false)">
                    <div class="p-6 border-b border-[var(--border-glass)] flex items-start gap-4">
                        <div
                            class="w-10 h-10 rounded-full {{ $isEditing ? 'bg-[var(--neon-amber)]/10 border-[var(--neon-amber)] text-[var(--neon-amber)]' : 'bg-[var(--neon-emerald)]/10 border-[var(--neon-emerald)] text-[var(--neon-emerald)]' }} flex items-center justify-center border shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1"
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-[var(--text-primary)] uppercase tracking-widest mb-1">
                                Confirmación</h3>
                            <p class="text-[var(--text-secondary)] text-xs leading-relaxed">
                                ...
                                @if($flightDurationHours > 0 && $airline)
                                    <br><br>
                                    <span class="text-[var(--neon-amber)] font-bold block mt-2 text-sm border-l-2 border-[var(--neon-amber)] pl-2">
                    ...
                    <div class="flex p-3 gap-3 bg-[var(--tech-input-bg)]">
                        <button type="button" wire:click="$set('showSaveModal', false)"
                            class="flex-1 py-2.5 px-4 text-xs font-bold text-[var(--text-secondary)] hover:text-[var(--text-primary)] bg-[var(--bg-panel)] rounded-[10px] border border-[var(--border-glass)] transition-colors">
                            Cancelar
                        </button>
                        <button type="button" wire:click="executeSave"
                            class="flex-1 py-2.5 px-4 text-xs font-bold text-black {{ $isEditing ? 'bg-[var(--neon-amber)] hover:bg-[var(--neon-amber)]/90' : 'bg-[var(--neon-emerald)] hover:bg-[var(--neon-emerald)]/90' }} rounded-[10px] transition-all">
                            Confirmar
                        </button>
                    </div>
                </div>
            ...
            <div
                ...
                <div class="tech-card bg-[var(--bg-panel)] max-w-sm w-full overflow-hidden shadow-2xl"
                    ...
                    <div class="p-6 border-b border-[var(--neon-rose)]/30 flex items-start gap-4">
                        <div
                            class="w-10 h-10 rounded-full bg-[var(--neon-rose)]/10 border border-[var(--neon-rose)] text-[var(--neon-rose)] flex items-center justify-center shrink-0">
                            <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1"
                        ...
                        <div>
                            <h3 class="text-sm font-bold text-[var(--neon-rose)] uppercase tracking-widest mb-1">Confirmar Eliminación
                            </h3>
                            <p class="text-[var(--text-secondary)] text-xs leading-relaxed">
                    ...
                    <div class="flex p-3 gap-3 bg-[var(--tech-input-bg)]">
                        ...
                        <button type="button" wire:click="executeDelete"
                            class="flex-1 py-2.5 px-4 text-xs font-bold text-white bg-[var(--neon-rose)] hover:bg-[var(--neon-rose)]/90 rounded-[10px] transition-all">
                            Eliminar
                        </button>
                    </div>
                </div>
        ...
        @if($showConflictDeleteModal)
            <div class="fixed inset-0 z-[70] flex items-center justify-center bg-black/90 backdrop-blur-xl p-4">
                <div
                    class="tech-card border-2 border-[var(--neon-rose)] max-w-md w-full overflow-hidden shadow-[0_0_50px_rgba(244,63,94,0.2)] animate-tech">
                    <div class="p-6 border-b border-[var(--neon-rose)]/30 flex items-start gap-4">
                        <div
                            class="w-12 h-12 rounded-full bg-[var(--neon-rose)] text-white flex items-center justify-center shrink-0 shadow-[0_0_20px_rgba(244,63,94,0.5)]">
                        ...
                        <div>
                            <h3 class="text-lg font-black text-[var(--neon-rose)] uppercase tracking-tighter mb-1">Conflicto de
                                Traslado detectado</h3>
                            <p class="text-[var(--text-secondary)] text-sm leading-relaxed">
                                Se han detectado <span
                                    class="text-[var(--text-primary)] font-bold underline">{{ $reservationsCount }} pasajeros ejecutivos</span> en este trayecto terrestre.
                            </p>
                            <p class="text-[var(--neon-rose)]/80 text-xs mt-2 italic font-medium leading-tight">
                    ...
                    <div class="flex p-4 gap-3 flex-col sm:flex-row bg-[var(--tech-input-bg)]">
                        <button type="button" wire:click="$set('showConflictDeleteModal', false)"
                            class="flex-1 py-3 px-4 text-xs font-bold text-[var(--text-secondary)] bg-[var(--bg-panel)] border border-[var(--border-glass)] rounded-[10px] uppercase tracking-widest">
                            Cancelar
                        </button>

                        <button type="button" wire:click="redirectToEdit"
                            class="flex-1 py-3 px-4 text-xs font-bold text-[var(--neon-amber)] hover:text-[var(--neon-amber)]/90 bg-[var(--neon-amber)]/10 border border-[var(--neon-amber)]/50 rounded-[10px] uppercase tracking-widest transition-all">
                            Editar
                        </button>

                        <button type="button" wire:click="cancelTerrestrialFlightAndNotify"
                            class="flex-1 py-3 px-4 text-xs font-bold text-white bg-[var(--neon-rose)] hover:bg-[var(--neon-rose)]/90 rounded-[10px] uppercase tracking-widest shadow-lg transition-all">
                            Eliminar
                        </button>
                    </div>
                </div>

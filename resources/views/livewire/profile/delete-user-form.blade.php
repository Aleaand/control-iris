...
<section class="p-8">
    <header class="mb-8">
        <div class="flex items-center gap-3 mb-2">
            ...
            <h2 class="text-xl font-black text-rose-500 uppercase tracking-[0.2em]">Eliminar Cuenta</h2>
        </div>
    </header>
    ...
    @if($isLastAdmin)
        <div class="p-4 bg-rose-500/10 border border-rose-500/20 rounded-xl mb-6">
            <p class="font-mono-tech text-[9px] text-rose-400 uppercase leading-relaxed">
                <span class="font-bold">[BLOQUEO]:</span> Eres el único administrador activo. El sistema requiere al menos
                un administrador para mantenerse funcional. La eliminación está deshabilitada.
            </p>
        </div>
    @endif

    <button x-data="" x-on:click.prevent="$dispatch('open-modal', 'confirm-user-deletion')" @if($isLastAdmin) disabled
    @endif
    ...
    <x-modal name="confirm-user-deletion" :show="$errors->isNotEmpty()" focusable>
        <form wire:submit="deleteUser"
            class="p-8 bg-[#0a0a0f] border border-rose-500/20 rounded-3xl overflow-hidden relative">
            <div
            ...
            <p class="font-mono-tech text-[10px] text-zinc-400 uppercase leading-relaxed mb-8">
                Esta acción es irreversible. Se eliminarán todos los registros de vuelo, reservaciones y credenciales
                asociadas a esta cuenta.
            </p>

            <div class="space-y-6">
                <div class="space-y-2">
                    <label class="font-mono-tech text-[10px] text-rose-400/70 uppercase tracking-widest">Nivel 1: Frase
                        de Seguridad</label>
                    ...
                    <input wire:model="confirmation" type="text"
                        class="tech-input w-full p-4 border-rose-500/20 focus:border-rose-500"
                ...
                <div class="space-y-2" x-data="{ show: false }">
                    <label for="password"
                        class="font-mono-tech text-[10px] text-rose-400/70 uppercase tracking-widest">Autorización del
                        Operador</label>
                    <div class="relative">
                        <input wire:model="password" id="password" :type="show ? 'text' : 'password'"
                            class="tech-input w-full p-4 pr-12 border-rose-500/20 focus:border-rose-500"
                            placeholder="Contraseña de confirmación" />
                        <button type="button" @click="show = !show"
                            class="absolute inset-y-0 right-4 flex items-center text-rose-400/50 hover:text-rose-500 transition-colors">
            ...
            <div class="mt-10 flex justify-end gap-4">
                <button type="button" x-on:click="$dispatch('close')"
                    class="px-6 py-2 bg-zinc-900 text-zinc-400 font-bold text-[10px] uppercase tracking-widest rounded-lg border border-white/5 hover:bg-zinc-800 transition-all">
                    Cancelar
                </button>

                <button type="submit"
                    class="px-6 py-2 bg-rose-600 hover:bg-rose-500 text-white font-black text-[10px] uppercase tracking-widest rounded-lg transition-all shadow-[0_0_20px_rgba(244,63,94,0.4)]">
